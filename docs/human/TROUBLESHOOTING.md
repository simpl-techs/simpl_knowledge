...
| `auto-update-skill` fallisce | File `requirements-agent-ci.txt` in root del repo (vedi `library-repo-template/`). Secret **`DEEPSEEK_API_KEY`** (org o **repository secret** se non avete org secrets). Opzionale variable `SKILL_AGENT_MODEL`. Log step `pip install`: file mancante; step `Run aider`: key mancante/invalida. |
