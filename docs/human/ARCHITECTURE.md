...
`simpl-knowledge` è un hub di distribuzione del contesto agentico. Non è un monolite applicativo: è un insieme di file Markdown, plugin Claude Code, regole Cursor, script e workflow.
...
### 1. Hub: `simpl-techs/simpl-knowledge`
...
Questo evita che `simpl-knowledge` diventi una copia manuale e obsoleta di tutte le librerie.
...
2. Su merge in `main` che tocca `SKILL.md`, **`sync-skill-to-marketplace`** (dal template) crea/aggiorna il plugin `*-context` in **`simpl-techs/simpl-knowledge`** tramite PR.
...
2. merge su `main`;
3. il workflow apre una PR su `simpl-techs/simpl-knowledge`;
...
- aggiorna la cache git locale di `simpl-knowledge`;
...
| Cosa | Dove | Note |
|------|------|------|
| Standard org-wide | `plugins/simpl-standards/skills/` | PR dirette su `simpl-techs/simpl-knowledge` |
...
2. **Push `main`** su `.agent/SKILL.md` → workflow apre PR su `simpl-techs/simpl-knowledge`.  
