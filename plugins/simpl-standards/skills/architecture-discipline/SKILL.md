...
- **Bump the version on every PR.** PATCH by default — including new public surface. MINOR only when the PR body justifies it, MAJOR only for breaking changes and only as a team decision. Full policy: `git-workflow` → *Version bumps*.
