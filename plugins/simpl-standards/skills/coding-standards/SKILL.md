...
- **Infra**: Docker + docker-compose for local; Google Cloud Run (services, worker pools, jobs, Prefect flows) and Vercel (Next.js apps) for prod
...
| Deploying or creating anything on Google Cloud (Cloud Run service, worker pool, job, Prefect flow): compute and LLM cost tracking is mandatory; a cost that cannot be recorded is reported on the cost-tracking channel, never a failure | `state-and-persistence` (Observability) + `simpl_tracker` |
