...
description: Promote a learned instinct (or all promotion-ready ones) into a real SKILL.md in this repo or into simpl_knowledge (simpl-standards), and mark it as promoted so it stops appearing in reminders.
---
...
- "Yes, and it's org-wide" → target = `plugins/simpl-standards/skills/<n>/SKILL.md` (in `simpl-techs/simpl_knowledge`)
...
The library repo’s sync workflow will open a PR on `simpl-techs/simpl_knowledge` automatically on merge (when the change includes `.agent/SKILL.md`).
