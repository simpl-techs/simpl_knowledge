...
Structured output mode is chosen per provider route: OpenRouter with
`require_parameters` uses native JSON schema, OpenAI-compatible routes use a
forced output tool, and `deepseek` always uses prompted output (DeepSeek's
models think by default and reject a forced `tool_choice`), whatever the model
name and whether or not `reasoning` is set — so `deepseek-flash` and
`deepseek-v4-pro` both work with tools out of the box.
