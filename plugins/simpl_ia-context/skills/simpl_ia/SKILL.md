...
Providers: `openai`, `deepseek`, `openrouter` (OpenRouter models are `vendor/model` slugs).
`client.selected_target_metadata(result)` reports which chain member answered
(`model`, `provider`, `reasoning`, `fallback_index`).
